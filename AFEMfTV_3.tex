...
%\usepackage[nopoints,nosolution]{ians_exercise}
...
%\renewenvironment{comment}{\newline\color{blue}\textnormal{\textbf{Comment:}} \itshape}{}
...
\setkomafont{caption}{\small\itshape} %Style von caption
\setkomafont{captionlabel}{\small\itshape\bfseries} %Style von Label von Caption
...
In this project we solve the constituted problems by constructing methods which automatically adjust to the discontinuities of the underlying (observed) data. This adjustment leads to methods, which adaptively (in an iterative manner) reduce the reconstruction error, allowing to obtain the discontinuities more accurately and hence yield qualitatively better reconstructions than state-of-the-art methods, while keeping the computational complexity of the underlying problem on a manageable size.%, and \hl{keep or even reduce the computational complexity} \todo{is this true?}  of the underlying problem at the same time. 
Additionally to these attributes the derived methods will possess further desirable properties, as robustness with respect to the given data %they are guaranteed to converge to a more accurate solution than state-of-the-art methods,
...
The principal investigator %and co-investigator 
...
%, finite element methods, a posteriori estimates and adaptive methods. 
...
%Here, $L^q(\Omega)$, with $q\in [1,\infty]$, denotes the usual Lebesgue space \cite{AdaFou} and $C_c^l(\Omega)$, $l\in \N$, is the space of $l$-times continuously differentiable functions with compact support in $\Omega$. In the event that $V(u,\Omega)<\infty$ 
...
%In image reconstruction one is interested in recovering an unknown image from a corrupted observation. 
...
The choice of the data-term typically depends on the type of noise contamination. For Gaussian noise contaminated images a quadratic $L^2$-data-term is usually chosen and has been successfully used; see for example \cite{BuSaSt:16} and references therein. %\cite{Cha, ChaDar, ChaLio, ChaGolMul, ComWaj, DarSig05, DarSig06, DauTesVes, DobVog, GolOsh, HinSta, Nes, OshBurGolXuYin, WeiBlaAub}. 
...
%There are two commonly used models of impulse noise considered in the literature. The first one, called \emph{salt-and-pepper noise}, where the noisy image $g$ is given by
%$$
%g(x) = 
%\begin{cases}
...
%\end{cases}
%$$
%with $c$ being a uniformly distributed random variable in the image intensity range $[d_{\min},d_{\max}]$. 
...
In the case  of non-smooth problems the works~\cite{DoMoNo:08,Bartels:15a} show an improvement of accuracy with respect to degrees of freedom in the case of Mumford-Shah and $L^2$-TV cases. %The general case of $L^1$-$L^2$-TV type has not been studied yet in adaptive meshes.
...
Instead of selecting the locally dependent weight $\lambda$ in \eqref{L2TVmultimodel2} in \citeO{Langer:17} a fully automatic selection strategy of locally adaptive parameters $\alpha$ for the problem \eqref{L2TVmultimodel} is presented. There the adjustment of $\alpha$ is based on local constraints, similarly as already considered for example in \cite{AlBaCaHa:08,DoHiRi:11,HintermullerRincon-Camacho:10}. However, while the iterative update of the spatially varying parameter in \cite{AlBaCaHa:08,DoHiRi:11,HintermullerRincon-Camacho:10} depends on an additional parameter, which needs to be chosen by the user, the approach in \citeO{Langer:17} does not need any additional parameter and hence is fully automatic for the user. Moreover, differently to the approaches in \cite{DoHiRi:11,HintermullerRincon-Camacho:10} where the initial regularization parameter $\lambda>0$ has to be set sufficiently small, in our approach any initial $\alpha>0$ is allowed. In this sense is our algorithm in \citeO{Langer:17} even more general than the ones presented in \cite{DoHiRi:11,HintermullerRincon-Camacho:10}.
...
All the above described locally adaptive parameter selection rules are related on \eqref{L2TV} and \eqref{L1TV} and hence are only suitable for removing Gaussian or impulsive noise and usually do not work satisfactory for a mixture of these noise types. For images, which are contaminated by a mixture of Gaussian and impulse noise, in \citeO{HintermullerLanger:13} an $L^1$-$L^2$-data-fidelity term has been introduced and shown to be suited to the task of removing this type of mixed noise simultaneously. In this approach, which we call $L^1$-$L^2$-TV model, an image is restored by solving
\begin{equation}\label{L1L2TVmodel}
\min_{u\in BV(\Omega)} \lambda_1 \|T_1u-g_1\|_{L^1(\Omega)} +  \lambda_2 \|T_2 u-g_2\|_{L^2(\Omega)}^2 + \alpha \int_{\Omega}|Du|,
\end{equation}
where $T_i : L^2(\Omega) \to L^2(\Omega)$ is a bounded linear operator, $g_i\in L^2(\Omega)$ is a given datum, and $\lambda_i\geq 0$ for $i=1,2$ with $\lambda_1+\lambda_2>0$. For the case of removing a mixture of Gaussian and impulse noise from an image $g$ one typically sets $T_1=T_2$ and $g_1=g_2=g$ in \eqref{L1L2TVmodel}. In this setting it is easy to see that the $L^1$-$L^2$-$TV$ model \eqref{L1L2TVmodel} is a generalization of \eqref{L2TV} and \eqref{L1TV}. In particular, if we set $\lambda_2=0$ in \eqref{L1L2TVmodel} then we obtain the $L^1$-TV model while for $\lambda_1=0$ we obtain the $L^2$-TV model. Clearly the solution of \eqref{L1L2TVmodel} highly depends on the parameters $\lambda_1$ and $\lambda_2$. A suitable selection of the parameters is discussed in \citeO{Langer:17a, Langer:19}.
...
For image denoising it has been demonstrated multiple times that a locally dependent regularisation parameter is able to produce better results than a globally constant parameter; cf. Section \ref{Sec:LocallyAdaptiveRegularization} and \ref{SecO:LocallyAdaptiveParameterSelection}. %In particular the local regularisation parameter should be large in homogeneous regions, while in parts with details the parameter should be small in order to preserve the structures. 
...
\begin{figure}
\begin{center}
\subfigure[Mesh-size of the non-uniform mesh. Twice refined around the jump.\label{fig:stepsize}]{\includegraphics[width=5cm]{stepsize}}\hspace{0.3cm}
\subfigure[Choice of the locally adaptive parameter.\label{fig:lambda1}]{\includegraphics[width=5cm]{lambda}}\hspace{0.3cm}
\subfigure[Zoom of the locally adaptive parameter.\label{fig:lambda2}]{\includegraphics[width=5cm]{lambda_zoomed}}
...
\min_{u\in L^2(\Omega)} \int_\Omega \lambda_1(x) | T_1 u - g_1| + \lambda_2(x) |T_2 u - g_2|^2 \text{d}x + \int_\Omega \alpha(x) |D u|, %R_{\alpha}(u), 
\end{equation}
where $\lambda_1,\lambda_2, \alpha :\Omega \to \R_0^+$. %and %$R_{\alpha}(u) = \int_\Omega \alpha(x) |\nabla u|^2 \text{d}x$ or $R_\alpha(u) = \int_\Omega \alpha(x) |D u|$ with
% $\alpha:\Omega \to \R^+$. %Although the optimization problem \eqref{Eq:LocalL1L2TV} allows for locally varying $\lambda_1, \lambda_2$, and $\alpha$, we will also consider special instances, e.g., only $\alpha$
...
In this part indicators telling in which locations of the image domain the grid should be refined or coarsened and how the locally varying regularization parameter(s) should be adjusted are derived. We start by constructing (heuristic) indicators for grid adaptation based on the idea that the mesh should be finer around edges than in relative uniform areas. At a later stage we also derive fully analytic motivated criteria based on error estimates. %Based on these indicators adaptive solution strategies are constructed.
 ...
 %in the beginning such a criterion might be heuristic, motivated by intuitive considerations or numerical observations. Let us describe two possible approaches:
%\begin{enumerate}
...
%\vspace{-0.5cm}
%\item 
As we are looking for a combined $h$-adaptive and locally adaptive parameter choice method, we may first select a locally adaptive parameter choice method, e.g., \cite{DoHiRi:11}. This method then already delivers an indicator for adjusting the parameters locally. This indicator will be investigated how it can be utilized to perform $h$-adaptivity. More precisely we believe that the indicator provided in \cite{DoHiRi:11} for the adjustment of the parameter can also be used for indicating $h$-adaptivity, as this indicator leads to parameters which regularize less around features while regularizing much in homogeneous regions, see \cite{DoHiRi:11}. Subsequently this approach is then also adjusted to other locally adaptive parameter selection methods, as \citeO{Langer:17} and \cite{CaLaPrSg:19}, and accompanied by a numerical comparison.
\begin{comment}
The local adaptive parameter choice is an iterative procedure. Such an indicator usually depends on the previously computed approximation and hence depends on the regularisation parameter. One has to be careful, as a small regularisation parameter leads to a global change of the parameter although locally differently. This needs to be taken into account.
\end{comment}
\begin{comment}
Additional approach (very heuristically): As already stated above, we wish the regularization parameter to be locally small at discontinuities, in order to preserve them. Hence it seems suitable that the mesh is fine around a discontinuity allowing to localize the jump accurately. In this vein, a possible indicator may be an edge indicator, as the Sobel operator or the Canny detector. A consequential strategy to obtain a suitable refinement indicator might be \begin{inparaenum}[(i)] \item (pre-)denoise the image by a standard technique \item apply an edge detector \item define the indicator according to the result of the edge detector\end{inparaenum}. This refinement indicator is then combined with a locally adaptive parameter selection strategy. As there exist several different locally adaptive parameter selection strategies, we may use one of these; see \citeO{Langer:17} \cite{DoHiRi:11, CaLaPrSg:19}, and compare them numerically.  %first use the one described in \cite{Lan}. Subsequently we will adjust the approach to other adaptive parameter selection strategies, as \cite{}, and compare them numerically.  \\\

Additional considerations to take into account: ad (i) How accurate does the pre-denoised solution need to be? Should it be over-smoothed (no noise but possibly features and edges are lost) or under-smoothed (still noisy but no features lost)? Which method is suitable (filter, like Gaussian filter, or an variational approach, as TV-denoising)? ad (ii) How accurate should the edge detector be? Is it preferable to use a reluctant one or should it detect nearly everything, which would also include noise? All these question lead to a question of philosophy: Should be refined in case of doubt, or not? From a quality point of view we should refine better more than too less, as this allows for a more precise adjustment of the regularization parameter. However this might be computationally too costly. This needs to be checked and acted accordingly!
\end{comment}
%\end{enumerate}  
%In both above described approaches 
...
\item Determine a respective dual problem $\sup_{p\in W^*} J^*(p)$, where $W^*$ is an associated dual space. Note that the dual problem is not unique and different possibilities might be considered. %Note, that by the Fenchel-duality principle we have $\inf_{u\in V} J(u)\geq \sup_{p\in W^*} J^*(p)$.
...
\item \textbf{Simultaneously:} As the local adaptive parameter selection methods are of iterative nature, we may incorporate the $h$-adaptivity into the parameter selection iterations. This means, the update of the parameter and the mesh adaptation is performed simultaneously. %performing one or a few iterations of the parameter selection method before applying the $h$-adaptivity.  
\begin{comment}
Formulate the problem as a bi-level optimisation problem (machine learning). Seems hard and would be a project on its own!
\end{comment}
\end{enumerate}
...
% We will analytically and numerically investigate such strategies with respect to convergence and restoration capabilities. 
...
\subparagraph{WP 3.1: Implementation of a finite difference method on non-uniform grids}
 ...
 The literature indicates that for solving the inverse problem of reconstructing an image from its observed corrupted data usually finite differences are used. In this vein we first implement a finite difference method for total variation denoising algorithms which can handle locally refined meshes. In particular we implement different solution strategies for solving the $L^2$-TV, $L^1$-TV and $L^1$-$L^2$-TV model on non-uniform grids. % the derived double adaptive algorithm by an adaptive finite difference method.  
 To the best of our knowledge, this seems to be the first finite difference approach on non-uniform grids for total variation minimization. The implementation of these algorithms form the basis of the locally adaptive parameter selection and $h$-adaptive method derived in WP 2, whose implementation, analysis and validation will be carried out in WP 3.2. % and WP 2.3. 
\begin{comment}
At this stage the grid is not yet automatically refined, but a non-uniform grid is chosen by hand.
\end{comment}


 \subparagraph{WP 3.2: Implementation of fully adaptive methods by finite differences}
 
 Based on the finite difference framework developed in WP 3.1, we will implement the different adaptive strategies derived in WP 1 and WP 2. In particular, first the strategies based on the heuristic indicators (WP 1.1) are considered, while in a later phase also the indicators based on error estimates (WP 1.2) are implemented. %Thereby the construction of the fully adaptive methods is done such that the resulting discrete algorithms converge and have desired properties, as stability and robustness, which should be theoretically justified. 
 %Note that the algorithms for the $L^2$-TV, $L^1$-TV  and $L^1$-$L^2$-TV models will be quite similar, as long as only one parameter is locally adjusted. In case of several locally varying parameters, a new derivation of an adaptive algorithm and consequently a theoretical investigation of the respective underlying optimization problem is needed.  
...
\begin{comment}  
A comparison of different methods in a FE setting could be announced as a master-thesis; same may apply for DG $\Rightarrow$ HiWi job.\\
\end{comment}
Due to the fact that the underlying solution is supposed to be discontinuous, we expect a suitable finite element space to be discontinuous, leading to a discontinuous Galerkin (DG) implementation. In fact, it is demonstrated in \citeO{Dieterich:19} that a DG implementation -- in case of the symmetric interior penalty method -- is able to preserve edges tremendously more accurate than a continuous Galerkin (CG) implementation. A key point in the DG implementation is that the penalty term is made spatially adaptive, so that the formulation is only penalized largely where the solution should be continuous. Motivated by these positive results performed on uncorrupted test images in \citeO{Dieterich:19}, in this project we will investigate how a DG implementation may be used efficiently to solve total variation image denoising problems. 
...
\begin{comment}
Description of software concept: DUNE; parallelisation; load balancing by space filling curves, ...
\end{comment}
...
\begin{comment}
In particular we consider the following settings:
\begin{enumerate}[(i)]
...
\end{figure}
\begin{comment}
 Additional question: How does an h-adaptive and locally weighted TV method compare with higher order regularizers, as TGV? A digital image is always piecewise constant!
\end{comment}
\begin{comment}
...
\hline
\textbf{WP 2 Adaptive strategies \& analysis} & & & \\ %\-\hspace{2pt} WP 6 
\-\hspace{2pt} WP 2.1  Adaptive strategies& \MyColorBox{1.5cm}{3pt}{0cm} &  & \\% \multicolumn{3}{l}{\textbullet\textbullet\textbullet\textbullet}   \\
...
\hline
\textbf{WP 3 Methods} & & &  \\
\-\hspace{2pt} WP 3.1  Adaptive FDM& \MyColorBox{0.5cm}{3pt}{0cm} &  & \\% \multicolumn{3}{l}{\textbullet\textbullet\textbullet\textbullet}   \\
...
Total requested allowance for travel: \EUR{10500}.
\begin{itemize}\itemsep=-1pt
\item Participation to one national (\EUR{700}) and one international (\EUR{1400}) conference per year. Duration trip: one week. Persons travelling: applicant  or doctoral researcher. Potential conferences: Enumath, MAFELAP,  SIAM Conference on Imaging Science, GAMM annual meeting, European Finite Element Fair. \\
...
Total requested allowance for transportation and accommodation of invited researchers: \EUR{4200}.
\begin{itemize}\itemsep=-1pt
...
\item[Andreas Langer:] PostDoc, Chair for Numerical Mathematics for High Performance Computing, Institute for Applied Analysis and Numerical  Simulation, Faculty for Mathematics and Physics, University of Stuttgart (budgetary funds, non-permanent until 30.06.2023)
\end{description}
%\begin{description}
...
%\subparagraph*{IANS (Dr. Andreas Langer)}
...
%\item Arthur G\"unthner, M.Sc. (financed by the DFG cluster of excellence ``Simulation Technology'', non-permanent until 30.11.2022)
  
  %  and Martin Alk\"amper MSc, doctoral student financed by the DFG cluster of excellence ``Simulation Technology'', who work intensively on an adaptive simulation environment for parabolic and elliptic problems in \dunefem.
\end{itemize}
...
\item Zakaria Belhachmi (University of Upper Alsace) %Name another external collaborator you plan to visit or send the doctorand.
...
Martin Alk\"amper (Stuttgart), Zakaria Belhachmi (Mulhous), Bernd Flemisch (Stuttgart), Fernando Gaspoz (Stuttgart), Claus-Justus Heine (Stuttgart), Roland Herzog (Chemnitz), Michael Hinterm\"uller (Berlin), %Pedro Morin (Santa Fe, Argentina),
 Talal Rahman (Bergen), Carlos Rautenberg (Berlin), Bin Wu (Bergen), Tao Wu (M\"unchen). %, Kunibert G. Siebert (Stuttgart), Andreas Veeser (Milan, Italy)
...
%List large instruments that will be available to you for the project. These may include large computer facilities if computing capacity will be needed.
